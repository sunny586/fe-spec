 ...
 * @param cwd
 * @param pkg
 * @returns sunny586-fe-eslint-config/index
 * @returns sunny586-fe-eslint-config/react
 * @returns sunny586-fe-eslint-config/typescript/index
 * @returns sunny586-fe-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'sunny586-fe-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
